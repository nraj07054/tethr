 ...
 */
class DogenGestureService : AccessibilityService() {
    ...
    companion object {
        @Volatile
        var instance: DogenGestureService? = null

        /** True once the user has enabled Dogen's control service in Settings. */
        fun isEnabled(context: Context): Boolean {
            val flat = Settings.Secure.getString(
                context.contentResolver,
                Settings.Secure.ENABLED_ACCESSIBILITY_SERVICES
            ) ?: return false
            val id = "${context.packageName}/${DogenGestureService::class.java.name}"
